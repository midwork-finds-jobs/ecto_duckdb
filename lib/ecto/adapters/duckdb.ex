defmodule Ecto.Adapters.DuckDB do
  @moduledoc """
  Adapter module for DuckDB.

  It uses `Duckex` for communicating to the database.

  ## Options

  The adapter supports a superset of the options provided by the
  underlying `Duckex` driver.
  ...
  ### Provided options
    ...
    * `:secure_delete` - Defaults to `:off`. Can be `:off` or `:on`. If `:on`, it will
      cause DuckDB to overwrite records that were deleted with zeros.
    ...
    * `:load_extensions` - list of paths identifying extensions to load. Defaults to `[]`.
       ...
       library compiled for the current architecture. See `Duckex.Connection.connect/1`
       for more.

  For more information about the options above, see [sqlite documentation][1]

  ### Differences between DuckDB and Ecto DuckDB defaults

  For the most part, the defaults we provide above match the defaults that DuckDB usually
  ships with. However, DuckDB has conservative defaults due to its need to be strictly
  backwards compatible, so some of them do not necessarily match "best practices". Below
  are the defaults we provide above that differ from the normal DuckDB defaults, along
  with rationale.

    * `:journal_mode` - we use `:wal`, as it is vastly superior
      for concurrent access. DuckDB usually defaults to `:delete`.
      See [DuckDB documentation][2] for more info.
    * `:temp_store` - we use `:memory`, which increases performance a bit.
      DuckDB usually defaults to `:file`.
    * `:foreign_keys` - we set it to `:on`, for better relational guarantees.
      This is also the default of the underlying `Duckex` driver.
      DuckDB usually defaults to `:off` for backwards compat.
    * `:busy_timeout` - we set it to `2000`, to better enable concurrent access.
      This is also the default of `Duckex`. DuckDB usually defaults to `0`.
    * `:cache_size` - we set it to `-64000`, to speed up access of data.
      DuckDB usually defaults to `-2000`.
  ...
  * Because DuckDB does not have a dedicated UUID type, most clients cannot represent
  ...
  is encouraged to also view the guidance on [when to use DuckDB][4] provided
  by the DuckDB documentation, as well.

  ### In memory robustness

  When using the Ecto DuckDB adapter with the database set to `:memory` it
  ...
  The Ecto DuckDB adapter does not support async tests when used with
  `Ecto.Adapters.SQL.Sandbox`. This is due to DuckDB only allowing up one write
  ...
  as [recommended by DuckDB][3]. This means you cannot do `LIKE` queries on `BLOB` columns.
  ...
  can make it case insensitive by using the [`COLLATE NOCASE`][6] option in DuckDB. This
  ...
  DuckDB supports specifying check constraints on the table or on the column definition.
  ...
  table-alter time. You can see more information in the DuckDB
  ...
  operation DuckDB does not support. You can however get the full functionality by
  adding a constraint at the column level, specifying the name and expression. Per the
  DuckDB documentation, there is no _functional_ difference between a column or table
  constraint.
  ...
  Unfortunately, unlike other databases, DuckDB does not provide the precise name of
  ...
  get the name of the violated constraint back from DuckDB at `INSERT` or `UPDATE`
  time, there is no way to effectively use these changeset functions. This is a DuckDB
  limitation.

  See [this GitHub issue](https://github.com/elixir-sqlite/ecto_duckdb/issues/42) for
  more details.

  ### Schemaless queries

  Using [schemaless Ecto queries][7] will not work well with DuckDB. This is because
  the Ecto DuckDB adapter relies heavily on the schema to support a rich array of Elixir
  types, despite the fact DuckDB only has [five storage classes][5]. The query will still
  ...
  **Set a global default:** Configure `:default_transaction_mode` to apply a 
  ...
  def storage_up(options) do
    ...
    cond do
      ...
      true ->
        ...
        # DuckDB will initialize it when first opened
        case File.touch(database) do
          :ok ->
  ...
  @impl Ecto.Adapter.Migration
  def lock_for_migrations(meta, options, fun) do
    # DuckDB uses file-level locking, so we don't need to acquire a separate lock
    # The database file itself acts as the lock
    # However, we need to ensure only one connection is used during migrations
  ...
  def dump_cmd(args, opts \\ [], config) when is_list(config) and is_list(args) do
    run_with_cmd("duckdb", ["-init", "/dev/null", config[:database] | args], opts)
  end
  ...
  def dumpers(:utc_datetime, type) do
    dt_type = Application.get_env(:ecto_duckdb, :datetime_type, @default_datetime_type)
  ...
  defp dump_schema(config) do
    case run_with_cmd("duckdb", [config[:database], ".schema"]) do
      {output, 0} -> {:ok, output}
      {output, _} -> {:error, output}
    end
  end
  ...
  defp extensions do
    Application.get_env(:ecto_duckdb, :type_extensions, [])
  end
