defmodule Ecto.Adapters.DuckDB.Connection do
  ...
  defp default_opts(opts) do
    # DuckDB only allows one writer at a time, so default to pool_size: 1
    # Users can override this if they know what they're doing (read-only connections)
    opts
  ...
  def start_link(opts) do
    ...
    DBConnection.start_link(Duckex.Protocol, opts)
  end
  ...
  def child_spec(options) do
    ...
    DBConnection.child_spec(Duckex.Protocol, options)
  end

  @impl true
  def prepare_execute(conn, name, sql, params, options) do
    query = %Duckex.Query{query: sql}

    case DBConnection.prepare_execute(conn, query, params, options) do
      {:ok, _, _} = ok -> ok
      {:error, %Duckex.Error{}} = error -> error
      {:error, err} -> raise err
    end
  end

  @impl true
  def execute(conn, %Duckex.Query{stmt: stmt} = cached, params, options)
      when stmt != nil do
  ...
  def execute(
        conn,
        %Duckex.Query{query: query_sql, stmt: nil},
        params,
        options
      ) do
    execute(conn, query_sql, params, options)
  end

  def execute(conn, sql, params, options) when is_binary(sql) or is_list(sql) do
    query = %Duckex.Query{query: IO.iodata_to_binary(sql)}

    case DBConnection.prepare_execute(conn, query, params, options) do
      {:ok, %Duckex.Query{}, result} -> {:ok, result}
      {:error, %Duckex.Error{}} = error -> error
      {:error, err} -> raise err
    end
  end
  ...
  # the constraint error, but in DuckDB as far as I can tell there
  ...
  @impl true
  def to_constraints(
        %Duckex.Error{message: "UNIQUE constraint failed: index " <> constraint},
        _opts
      ) do
  ...
  def to_constraints(
        %Duckex.Error{message: "UNIQUE constraint failed: " <> constraint},
        _opts
      ) do
    [unique: constraint_name_hack(constraint)]
  end

  def to_constraints(%Duckex.Error{message: "FOREIGN KEY constraint failed"}, _opts) do
    # unfortunately we have no other date from DuckDB
    [foreign_key: nil]
  end

  def to_constraints(
        %Duckex.Error{message: "CHECK constraint failed: " <> name},
        _opts
      ) do
    [check: name]
  end
  ...
  def delete_all(%Ecto.Query{joins: [_ | _]}) do
    ...
    raise ArgumentError, "JOINS are not supported on DELETE statements by DuckDB"
  end
  ...
  def execute_ddl({:create, %Table{} = table, columns}) do
    ...
    # Generate sequence statements for serial/bigserial columns
    sequence_sqls = generate_sequence_statements(table, columns)

    [create_table_sql | sequence_sqls]
  end
  ...
  defp generate_sequence_statements(%Table{name: table_name, prefix: prefix}, columns) do
    columns
    |> Enum.filter(fn
      {:add, _name, type, _opts} when type in [:serial, :bigserial] -> true
      _ -> false
    end)
    |> Enum.flat_map(fn {:add, column_name, _type, _opts} ->
      sequence_name = "#{table_name}_#{column_name}_seq"
      quoted_table = quote_table(prefix, table_name)
      quoted_column = quote_name(column_name)

      [
        # Create the sequence
        ["CREATE SEQUENCE ", quote_name(sequence_name), " START 1"],
  ...
  defp on_conflict({:replace_all, _, []}, _header) do
    raise ArgumentError, "Upsert in DuckDB requires :conflict_target"
  end
  ...
  def insert_each(values, counter) do
    intersperse_reduce(values, ?,, counter, fn
      nil, _counter ->
        raise ArgumentError,
              "Cell-wise default values are not supported on INSERT statements by DuckDB"
  ...
  defp update_op(command, _quoted_key, _value, _sources, query) do
    raise Ecto.QueryError,
      query: query,
      message: "Unknown update operation #{inspect(command)} for DuckDB"
  end
  ...
  defp order_by_expr({dir, expression}, sources, query) do
    ...
    case dir do
      ...
      _ ->
        raise Ecto.QueryError,
          query: query,
          message: "#{dir} is not supported in ORDER BY in DuckDB"
    end
  end
  ...
  # workaround for the fact that DuckDB as of 3.35.4 does not support specifying table
  ...
  defp expr({:fragment, _, [kw]}, _sources, query)
    ...
    raise Ecto.QueryError,
      query: query,
      message: "DuckDB adapter does not support keyword or interpolated fragments"
  end
  ...
  defp expr({:count, _, [{:&, _, [_]}]}, _sources, query) do
    raise Ecto.QueryError,
      query: query,
      message: "The argument to `count/1` must be a column in DuckDB"
  end
  ...
  def interval(_, "microsecond", _sources) do
    raise ArgumentError,
          "DuckDB does not support microsecond precision in datetime intervals"
  end
  ...
  # If we are adding a DATETIME column with the NOT NULL constraint, DuckDB
  ...
  defp column_change(_table, _) do
    raise ArgumentError, "Not supported by DuckDB"
  end
