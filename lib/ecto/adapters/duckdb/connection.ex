defmodule Ecto.Adapters.DuckDB.Connection do
  ...
  def start_link(opts) do
    ...
    DBConnection.start_link(Duckdbex.Protocol, opts)
  end
  ...
  def child_spec(options) do
    ...
    DBConnection.child_spec(Duckdbex.Protocol, options)
  end

  # TODO: Handle the name properly
  @impl true
  def prepare_execute(conn, _name, sql, params, options) do
    query = %Duckdbex.Query{query: sql, stmt: nil, columns: nil, rows: nil}

    case DBConnection.prepare_execute(conn, query, params, options) do
      {:ok, _, _} = ok -> ok
      {:error, %Duckdbex.Error{}} = error -> error
      {:error, err} -> raise err
    end
  end

  @impl true
  def execute(conn, %Duckdbex.Query{stmt: stmt} = cached, params, options)
      when stmt != nil do
  ...
  def execute(
        conn,
        %Duckdbex.Query{query: query, stmt: nil},
        params,
        options
      ) do
  ...
  def execute(conn, sql, params, options) when is_binary(sql) or is_list(sql) do
    query = %Duckdbex.Query{query: IO.iodata_to_binary(sql), stmt: nil, columns: nil, rows: nil}

    case DBConnection.prepare_execute(conn, query, params, options) do
      {:ok, %Duckdbex.Query{}, result} -> {:ok, result}
      {:error, %Duckdbex.Error{}} = error -> error
      {:error, err} -> raise err
    end
  end
  ...
  @impl true
  def to_constraints(
        %Duckdbex.Error{message: "UNIQUE constraint failed: index " <> constraint},
        _opts
      ) do
  ...
  def to_constraints(
        %Duckdbex.Error{message: "UNIQUE constraint failed: " <> constraint},
        _opts
      ) do
    [unique: constraint_name_hack(constraint)]
  end

  def to_constraints(%Duckdbex.Error{message: "FOREIGN KEY constraint failed"}, _opts) do
    # unfortunately we have no other date from DuckDBex
    [foreign_key: nil]
  end

  def to_constraints(
        %Duckdbex.Error{message: "CHECK constraint failed: " <> name},
        _opts
      ) do
    [check: name]
  end
  ...
  def table_exists_query(table) do
    {"SELECT table_name FROM information_schema.tables WHERE table_name = ? LIMIT 1", [table]}
  end
  ...
  defp on_conflict({:replace_all, _, []}, _header) do
    raise ArgumentError, "Upsert in DuckDBex requires :conflict_target"
  end
  ...
  def insert_each(values, counter) do
    intersperse_reduce(values, ?,, counter, fn
      nil, _counter ->
        raise ArgumentError,
              "Cell-wise default values are not supported on INSERT statements by DuckDBex"
  ...
  defp update_op(command, _quoted_key, _value, _sources, query) do
    raise Ecto.QueryError,
      query: query,
      message: "Unknown update operation #{inspect(command)} for DuckDBex"
  end
  ...
  defp order_by_expr({dir, expression}, sources, query) do
    ...
    case dir do
      ...
      _ ->
        raise Ecto.QueryError,
          query: query,
          message: "#{dir} is not supported in ORDER BY in DuckDBex"
    end
  end
  ...
  defp combination({:intersect_all, query}, _) do
    raise Ecto.QueryError,
      query: query,
      message: "DuckDBex does not INTERSECT ALL"
  end
  ...
  # workaround for the fact that DuckDBex as of 3.35.4 does not support specifying table
  ...
  defp expr({:fragment, _, [kw]}, _sources, query)
    ...
    raise Ecto.QueryError,
      query: query,
      message: "DuckDBex adapter does not support keyword or interpolated fragments"
  end
  ...
  defp expr({:count, _, [{:&, _, [_]}]}, _sources, query) do
    raise Ecto.QueryError,
      query: query,
      message: "The argument to `count/1` must be a column in DuckDBex"
  end
  ...
  defp column_change(_table, _) do
    raise ArgumentError, "Not supported by DuckDBex"
  end
  ...
  # DuckDBex doesn't use AUTOINCREMENT like SQLite
  # It uses sequences which are created automatically for INTEGER PRIMARY KEY columns
  defp pk_expr(true, _type), do: " PRIMARY KEY"
  defp pk_expr(_, _), do: []
