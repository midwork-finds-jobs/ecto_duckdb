defmodule Ecto.Adapters.DuckDB.Connection.FromTest do
  ...
  describe "without schema" do
    ...
    test "raises when selecting all fields without a schema" do
      assert_raise Ecto.QueryError,
                   ~r"DuckDB does not support selecting all fields from \"posts\" without a schema",
                   fn ->
  ...
  test "from with fragment" do
    ...
    assert_raise Ecto.QueryError, ~r/^DuckDB does not support/, fn ->
      from(f in fragment("select ? as x", ^"abc"))
      |> select([f], f)
      |> plan()
      |> all()
